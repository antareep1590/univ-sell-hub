...
type PayoutMethodForm = z.infer<typeof bankSchema>;
...
const ManagePayouts = () => {
  ...
  const handleAddMethod = (data: PayoutMethodForm) => {
    if (kycStatus !== 'verified') {
      toast({
        title: "KYC Required",
        description: "Please complete your KYC verification before adding payout methods.",
        variant: "destructive"
      });
      return;
    }
    ...
    const action = () => {
      const newMethod: PayoutMethod = {
        ...
        type: 'bank',
        name: 'Bank Transfer',
        details: `${data.bankName} ***${data.accountNumber.slice(-4)}`,
  ...
  const handleKYCSubmit = (status: 'pending' | 'verified') => {
    setKycStatus(status);
  };
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">
                  Add and manage your bank account for receiving earnings
                </p>
              </div>
              <div className="flex items-center space-x-3">
                ...
                <Button
                  ...
                  variant={kycStatus === 'none' ? 'default' : 'outline'}
                >
                  <Shield className="h-4 w-4 mr-2" />
                  {kycStatus === 'none' ? 'Complete KYC' : kycStatus === 'pending' ? 'KYC Pending' : 'KYC Verified'}
                </Button>
                <Dialog open={isAddingMethod} onOpenChange={setIsAddingMethod}>
                  <DialogTrigger asChild>
                    <Button disabled={kycStatus !== 'verified'}>
